...
float sphereRotation(vec3 p, float r,vec3 rotation, float timer) //radius
{
    ...
    vertex = vertex * rotationZ(rotation.x)  * rotationY(rotation.y) * rotationX(rotation.z) * timer;
...
float repCross( in vec3 p, in vec3 c, float size)
{
    vec3 q = mod(p,c)-0.5*c;

    float da = box(q.xyz,vec3(10,size,size));
    float db = box(q.yzx,vec3(size,10,size));
    float dc = box(q.zxy,vec3(size,size,10));
...
{
    float k = 0.1 * sin(timer) + 0.1; // or some other amount
    float c = cos(k*p.y);
    float s = sin(k*p.y);
    mat2  m = mat2(c,-s,s,c);
    vec3  q = vec3(m*p.xz,p.y);
    return  repCross(q, vec3(2.0,2.0,2.0),0.1);
}

float map(vec3 p)
{
    vec4 vertex = vec4(p, 1.0);
    vec3 rotation = vec3(0.4,0.1,0.04);
    vertex = vertex * rotationZ(pow(timer,1/3)*rotation.x)  * rotationY(pow(timer,1/3)*rotation.y) * rotationX(pow(timer,1/3)*rotation.z);

    float mixV = sin(timer) * 0.5 + 0.5;

    return repCross(vertex.xyz, vec3(2.0,2.0,2.0),0.1);
    //return opTwist(vertex.xyz);
}
...
//    float the = iTime * 0.25;
 //   r.xz *= mat2(cos(the), -sin(the), sin(the), cos(the));
    ...
    o = vec3(0.0,0.0,timer/2);
    float t = trace(o, r);
    vec3 p = o + r*t;
   // vec3 N = estimateNormal(p, timer);
    
    vec3 K_a = vec3(0.2, 0.2, 0.2);
    //vec3 K_d = vec3(0.4+0.2*sin(timer/5), 0.4+0.2*sin(timer),0.4+0.2*sin(timer*3));
